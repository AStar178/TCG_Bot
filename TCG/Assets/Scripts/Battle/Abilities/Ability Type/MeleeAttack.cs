...
public abstract class MeleeAttack : Ability
{
    ...
    {
        Tween tween = battle.PlayerKnight.gameObject.transform.DOMoveX(battle.transform.position.x + 3, Util.Setting.Speed * .5f);
        ...
        battle.PlayerKnight.gameObject.transform.DORotate(new Vector3(0, 0, 0), Util.Setting.Speed * .5f);
        tween = battle.PlayerKnight.gameObject.transform.DOJump(new Vector3(battle.transform.position.x - 1, -3.5f, 0), 1,1,1);
        await tween.AsyncWaitForCompletion();
