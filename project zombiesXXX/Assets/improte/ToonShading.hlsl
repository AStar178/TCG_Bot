void ToonShading_float(in float3 Normal, in float ToonRampSmoothness, in float3 ClipSpacePos, in float3 WorldPos, in float4 ToonRampTinting, in float4 shadow,
in float ToonRampOffset, out float3 ToonRampOutput, out float3 Direction)
{
	...
	// set the shader graph node previews
		...
		toonRamp *= light.shadowAttenuation - shadow;
